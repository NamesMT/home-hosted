...
<template>
  <DialogRoot v-model:open="open">
    <DialogPortal>
      ...
      <DialogContent
        :class="cn(
          'fixed left-1/2 top-1/2 z-50 max-h-[88dvh] -translate-x-1/2 -translate-y-1/2 overflow-hidden',
      ...
      >
        ...
        <div class="max-h-[calc(88dvh-3.25rem)] overflow-y-auto px-4 py-3.5">
          <slot />
        </div>
        <footer v-if="$slots.footer" class="flex items-center justify-end gap-2 border-t border-line bg-panel-2 px-4 py-3">
          <slot name="footer" />
        </footer>
      </DialogContent>
    </DialogPortal>
